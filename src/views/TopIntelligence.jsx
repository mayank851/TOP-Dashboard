...
import {
  ...
  Legend, CartesianGrid, LineChart, AreaChart, Area,
} from 'recharts';
import {
...
const LOCATIONS = ['All', 'HSR', 'MTH', 'BTM', 'IND'];
const PLATFORMS_LIST = ['All', 'Swiggy', 'Zomato'];
...
export default function TopIntelligence({ records, brand }) {
  const [loc, setLoc] = React.useState('All');
  const [plat, setPlat] = React.useState('All');

  // Filter to selected brand (defaults to TOP)
  const brandRecords = filterRecords(records, {
    brand: brand || 'TOP',
    location: loc !== 'All' ? loc : undefined,
    platform: plat !== 'All' ? plat : undefined,
  });

  const weekly = brandRecords.filter(r => r.periodType === 'Weekly');
  const totals = sumRecords(weekly);
  const trend = getPeriodTrend(brandRecords);
  const locSummary = getLocationSummary(records, brand || 'TOP');
  const platSummary = getPlatformSummary(records, brand || 'TOP');
  const waterfall = getPayoutWaterfall(totals);

  // Period-over-period (last period vs prior)
  const sortedPeriods = [...new Set(weekly.map(r => r.periodStart))].sort();
  const lastPeriod = sortedPeriods[sortedPeriods.length - 1];
  const priorPeriod = sortedPeriods[sortedPeriods.length - 2];
  const lastRecs = sumRecords(weekly.filter(r => r.periodStart === lastPeriod));
  const priorRecs = priorPeriod ? sumRecords(weekly.filter(r => r.periodStart === priorPeriod)) : null;
  ...
  // Available locations for this brand
  const brandLocs = ['All', ...new Set(records.filter(r => r.brand === (brand || 'TOP')).map(r => r.location))].filter(Boolean);

  return (
    <div className="content-area">
      <div className="view-header">
        <div className="view-title">🏆 {brand || 'TOP'} Intelligence</div>
        <div className="view-subtitle">
          Seed / Series A metrics — payout intelligence · {sortedPeriods.length} periods
        </div>
      </div>
      ...
      <div className="kpi-grid kpi-grid-5" style={{ marginBottom: 12 }}>
        <KPICard label="GMV (Total)" value={fINR(totals.gmv, true)} color="purple" sub="Gross merchandise value" />
        <KPICard label="Net Payout" value={fINR(totals.netPayout, true)} color="green" changePct={netChange} sub="vs prior period" />
        <KPICard label="Net Sales" value={fINR(totals.netSales, true)} color="blue" sub="After discount" />
        <KPICard label="Orders" value={fNum(totals.deliveredOrders)} color="teal" changePct={ordersChange} sub="vs prior period" />
      ...
      <div className="kpi-grid kpi-grid-5" style={{ marginBottom: 24 }}>
        <KPICard label="Net Margin %" value={fPct(totals.netMarginPct)} color={totals.netMarginPct > 20 ? 'green' : totals.netMarginPct < 10 ? 'red' : 'gold'} sub="Net payout / GMV" />
        ...
        <KPICard label="Discount %" value={fPct(totals.discountPct)} color="blue" sub="Discount share / GMV" />
      ...
      <div className="chart-card" style={{ marginBottom: 12 }}>
        <div className="chart-card-title">GMV & Net Payout — Week by Week</div>
      ...
      <div className="chart-card" style={{ marginBottom: 12 }}>
        <div className="chart-card-title">Unit Economics — Commission% · Ads% · Net Margin%</div>
      ...
      {/* Platform + Location side by side */}
      <div className="section-title">Platform & Location Split</div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {platSummary.map(p => (
                <tr key={p.platform}>
                  <td>
                    <span className={`badge-platform badge-${p.platform.toLowerCase()}`}>
                      {p.platform}
                    </span>
                  </td>
          ...
          <div key={i} className="waterfall-row">
            ...
            <div className="waterfall-bar-track">
              <div
                ...
                style={{ width: `${Math.min(barWidth, 100)}%`, background: row.color }}
              />
            </div>
